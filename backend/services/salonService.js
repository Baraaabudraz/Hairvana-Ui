...
exports.getSalonById = async (id, req) => {
  ...
  // Enforce ownership for salon owners
  if (req.user && req.user.role === "salon" && req.user.id) {
    if (!salon || String(salon.owner_id) !== String(req.user.id)) {
      return null;
    }
  }
...
exports.updateSalon = async (id, data, req) => {
  ...
  let gallery =
    oldSalon && Array.isArray(oldSalon.gallery) ? [...oldSalon.gallery] : [];
  if (req.files && req.files["avatar"] && req.files["avatar"][0]) {
    // Delete old avatar if it exists
    if (avatar) {
      const oldAvatarPath = path.join(
        __dirname,
        "../public/uploads/salons",
        avatar
      );
  ...
  if (oldSalon && Array.isArray(oldSalon.gallery)) {
    const removedImages = oldSalon.gallery.filter(
      (img) => !gallery.includes(img)
    );
