...
function serializeUser(user, options = {}) {
  ...
  const baseData = {
    ...
    email: user.email,
    phone: user.phone,
    avatar: avatarFilenameOnly
      ? user.avatar
      : user.avatar &&
        !user.avatar.startsWith("http") &&
        !user.avatar.startsWith("blob:") &&
        !user.avatar.match(/^[\w\.-]+\.[a-zA-Z0-9]+$/)
      ? `${options.req?.protocol || "http"}://${
          options.req?.get ? options.req.get("host") : "localhost:5000"
        }${user.avatar}`
      : user.avatar,
...
function getContextSpecificData(user, context, currentUser) {
  ...
  switch (context) {
    ...
    case "admin":
      ...
      contextData.join_date =
        user.join_date || user.createdAt || user.created_at;
