...
exports.login = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ where: { email, role: 'salon' } });
    if (!user) {
      return res.status(401).json({ error: 'Invalid credentials.' });
    }
    if (user.status === 'pending') {
      return res.status(403).json({ error: 'Account is pending approval. Please upload required documents or wait for admin approval.' });
    }
...
exports.uploadDocuments = async (req, res) => {
  try {
    ...
    if (!req.files || !req.files.commercial_registration || !req.files.certificate) {
      return res.status(400).json({ error: 'Both commercial registration and certificate are required.' });
    }
