...
export async function fetchSalons(params: { status?: string; search?: string; ownerId?: string } = {}) {
  try {
    ...
    return await apiFetch(`/backend/api/salons?${queryParams.toString()}`);
  } catch (error) {
...
export async function fetchSalonById(id: string) {
  try {
    return await apiFetch(`/backend/api/salons/${id}`);
  } catch (error) {
...
export async function createSalon(salonData: any) {
  try {
    return await apiFetch('/backend/api/salons', {
      method: 'POST',
...
export async function fetchSalonServices(salonId: string) {
  try {
    return await apiFetch(`/backend/api/salons/${salonId}/services`);
  } catch (error) {
...
export async function fetchSalonStaff(salonId: string) {
  try {
    return await apiFetch(`/backend/api/salons/${salonId}/staff`);
  } catch (error) {
...
export async function fetchSalonAppointments(salonId: string, params: { status?: string; from?: string; to?: string } = {}) {
  try {
    ...
    return await apiFetch(`/backend/api/salons/${salonId}/appointments?${queryParams.toString()}`);
  } catch (error) {
...
export async function updateSalonStatus(id: string, status: 'active' | 'pending' | 'suspended') {
  try {
    return await apiFetch(`/backend/api/salons/${id}/status`, {
      method: 'PATCH',
