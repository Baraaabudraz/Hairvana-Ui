...
exports.update = async (id, data) => {
  const [affectedRows, [updatedSalon]] = await Salon.update(data, { where: { id }, returning: true });
  return updatedSalon;
};
...
exports.updateStatus = async (id, status) => {
  const [affectedRows, [updatedSalon]] = await Salon.update({ status }, { where: { id }, returning: true });
  return updatedSalon;
};

exports.getRevenue = async (salonId) => {
  const revenue = await Appointment.sum('total_price', { where: { salon_id: salonId, status: ['booked', 'completed'] } });
  return revenue || 0;
};

exports.getBookings = async (salonId) => {
  const bookings = await Appointment.count({ where: { salon_id: salonId, status: ['booked', 'completed'] } });
  return bookings || 0;
};
...
exports.getRating = async (salonId) => {
  ...
  return ratingResult && ratingResult.dataValues.avgRating ? parseFloat(ratingResult.dataValues.avgRating) : 0;
};
